...
public function getMaterialInfo($id){ //am here 
    $query = $this->db -> prepare("SELECT * FROM material_tbl WHERE id = ? LIMIT 1");
...
<?php
    ...
    public function displaySalesSearchDate($startDate,$endDate)
    {
        $totalCost = 0;
     
        $query = $this->db->prepare("SELECT * FROM sales_tbl WHERE transactiondate BETWEEN ? AND  ? ORDER BY id DESC");
        ...
        if ($query->rowCount() > 0) {
            ...
            ?>
<div class="table-responsive">
<h3 class=""> Sales from <?php echo $startDate.' to '.$endDate; ?></h3>
    <table class="table table-hover table-bordered">
...
<?php
    ...
    public function displayUseMaterialSearchDate($startDate,$endDate)
    {
        $totalKg = 0;
     
        $query = $this->db->prepare("SELECT *,  SUM( kg ) AS kgg FROM  materialuse_tbl WHERE createdat BETWEEN ? AND  ?  GROUP BY materialid ");
        $query->execute(array($startDate,$endDate));
        if ($query->rowCount() > 0) {
            ...
            ?>
<div class="table-responsive">
<h3 class=""> Materials from <?php echo $startDate.' to '.$endDate; ?></h3>
    <table class="table table-hover table-bordered">

        <thead>
            <tr>
                <th>Material</th>
                <th>Total KG of Each Material Used</th>
                <th>Created At</th>
        ...
        while ($row = $query->fetch(PDO::FETCH_ASSOC)) {
            ...
            <tr>
                <td> 
                <?php  echo $this->getMaterialInfo($materialId)->materialname ;; ?>
                </td>
...
<?php
            ...
            ?>
             ...
             <tbody>
            <tr>
                <td colspan="1" style="text-align:right;font-size:20px; color:red;">
                    <span>Total KG</span>
                </td>
                <td >
                <span style="font-size:20px; color:red;">
...
<?php
    ...
    public function displayUseMaterial()
    {
        $totalKg = 0;
     
        $query = $this->db->prepare("SELECT *,  SUM( kg ) AS kgg FROM materialuse_tbl GROUP BY materialid");
        $query->execute();
        if ($query->rowCount() > 0) {
...
<?php
            ...
            <button type="button" onclick="window.print();return false;" class="btn btn-primary text-left" name="">Print Material Info</button>
            </div>
...
<?php
    ...
    public function displayMaterialSearchDate($startDate,$endDate)
    {
        $totalCost = 0;
